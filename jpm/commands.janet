...
(defn help
  ...
  (print 
    ...
    To install/manage packages in a local subtree, use the --local
    flag (or -l) to install packages to ./jpm_tree. This should
    generally not require elevated privileges.
    ...
    Privileged global subcommands:
        ...
        install (repo or name)...
            Install artifacts. If a repo is given, install the
            contents of that git repository, assuming that the
            repository is a jpm project. If not, build and install the
            current project.
        ...
        make-lockfile (lockfile)
            ...
            lockfile will record the exact versions of dependencies
            used to ensure a reproducible build. Lockfiles are best
            used with applications, not libraries. The default
        ...
        janet
            Run the Janet interpreter with JANET_PATH set to the
            correct module tree.
    ...
    Unprivileged project subcommands:
        ...
        test
            Run tests. Tests should be .janet files in the test/
            directory relative to project.janet.
            
        run rule

            Run a rule. Can also run custom rules added via
            `(phony "task" [deps...] ...)` or
            `(rule "ouput.file" [deps...] ...)`.
        ...
        rule-tree (root rule) (depth)
            ...
            Optionally provide a root rule to start printing from, and
            a max depth to print. Without these options, all rules
            will print their full dependency tree.
                                      
        debug-repl
            Run a repl in the context of the current project.janet
            file. This lets you run rules and otherwise debug the
            current project.janet file.
